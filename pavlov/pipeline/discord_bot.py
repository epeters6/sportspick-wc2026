...
async def send_signal(
    ...
    if webhook_alerts.webhook_enabled():
        await webhook_alerts.post_weather_signal(signal, venue="kalshi")
        logger.info(
            "DiscordBot: Kalshi signal via webhook for %s.", signal.get("ticker")
        )
        return
...
async def _sync_slash_commands_background() -> None:
    ...
    delay = max(5, int(os.environ.get("DISCORD_SYNC_DELAY_SECONDS", "20")))
...
async def on_ready() -> None:
    ...
    logger.info(
        "DiscordBot: logged in as %s (id=%s). Channel: %s. Poly channel: %s. Slash sync: background (throttled).",
        client.user,
    ...
    print(
        f"Discord bot ready — channel: {_channel}  |  poly: {_poly_channel}  |  slash sync: throttled background"
    )
