...
pavlov-mlb-bot — Discord + MLB Polymarket scheduler.
  ...
  python main.py run      Live: Discord + pregame cycles + optional in-game Poly poll
...
async def run_cycle() -> None:
    ...
    if channel is not None or use_webhook:
        for sig in signals:
            if not mlb_alerts.try_mark_alert(sig):
                continue
    ...
    try:
        await loop.run_in_executor(
            None, mlb_poly_discord.expire_stale_pending_mlb_signals
        )
    except Exception as exc:
        logger.warning("run_cycle: pending auto-skip scan failed — %s", exc)
...
async def run_mlb_ingame_poll() -> None:
    """Fetch live linescores + sports markets; place at most one in-game auto-bet."""
    from functools import partial

    from pipeline.mlb_ingame_bets import build_ingame_signals
    from polymarket.mlb_poly_discord import (
        maybe_auto_bet_mlb_ingame_from_candidates,
        mlb_poly_ingame_autobet_enabled,
    )

    if not mlb_poly_ingame_autobet_enabled():
        return
    ...
    def _markets() -> list:
        if not polymarket_client.poly_configured():
            return []
        try:
            return polymarket_client.get_markets(category="sports")
        except Exception as exc:
            logger.warning("run_mlb_ingame_poll: markets — %s", exc)
            return []

    markets = await loop.run_in_executor(None, _markets)
    if not markets:
        return

    cands = await loop.run_in_executor(
        None,
        partial(build_ingame_signals, games, markets),
    )
    if cands:
        channel = discord_bot.get_mlb_poly_channel() or discord_bot.get_channel()
        if channel is not None:
            for sig in cands:
                if not mlb_alerts.try_mark_alert(sig):
                    continue
                await discord_bot.send_mlb_signal(channel, sig)
        await maybe_auto_bet_mlb_ingame_from_candidates(cands[:1])


async def pending_auto_skip_loop() -> None:
    """Expire MLB Discord pending alerts with no BET/SKIP after the configured window."""
    ...
    while True:
        try:
            ...
            logger.exception("pending_auto_skip_loop: %s", exc)
        mins = max(
            5,
            int(CONFIG.get("MLB_PENDING_AUTO_SKIP_CHECK_MINUTES") or 10),
        )
        await asyncio.sleep(mins * 60)


async def ingame_poller_loop() -> None:
    """Periodic in-game opportunity scan (separate from 10am/4pm pregame cycle)."""
    from polymarket.mlb_poly_discord import mlb_poly_ingame_autobet_enabled
    ...
    while True:
        if not mlb_poly_ingame_autobet_enabled():
            ...
            continue
        try:
            await run_mlb_ingame_poll()
        except Exception as exc:
            logger.exception("ingame_poller_loop: %s", exc)
        mins = max(3, int(CONFIG.get("POLY_MLB_INGAME_POLL_MINUTES") or 12))
        await asyncio.sleep(mins * 60)
...
async def main_live() -> None:
    ...
    try:
        from polymarket import mlb_autobet_state as _mas

        raw_su = os.environ.get("POLY_MLB_AUTOBET_STARTUP_GRACE_SECONDS", "").strip()
        if raw_su:
            sec = float(raw_su)
            if sec > 0:
                _mas.arm_autobet_suppress(seconds=sec)
                logger.info(
                    "MLB: autobet suppressed %.0fs after startup "
                    "(POLY_MLB_AUTOBET_STARTUP_GRACE_SECONDS)",
                    sec,
                )
    except Exception as exc:
        logger.warning("MLB: startup autobet grace skipped — %s", exc)

    logger.info(
        "pavlov-mlb-bot live — ET 10:00/16:00 pregame | in-game poll (if enabled) | 00:00 resolve | 07:00 summary | "
...
def cmd_cycle_once(_a: argparse.Namespace) -> None:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser("cycle-once", help="One pregame cycle (GHA / webhook mode)")
