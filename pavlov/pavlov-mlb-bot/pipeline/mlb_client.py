...
def get_live_game_snapshot(game_pk: int) -> dict[str, Any] | None:
    """If the game is *live*, return scoreboard fields; else ``None``.

    Uses ``liveData.linescore`` from the v1.1 live feed. Safe to call every poll;
    suppresses errors and returns ``None``.
    """
    try:
        feed = get_game_feed_live(int(game_pk))
    except Exception as exc:
        logger.debug("mlb_client: live snapshot failed for %s — %s", game_pk, exc)
        return None
    gd = feed.get("gameData") or {}
    status = gd.get("status") or {}
    abstract = str(status.get("abstractGameState") or "").lower()
    detailed = str(status.get("detailedState") or "").lower()
    if abstract != "live" and "in progress" not in detailed and "progress" not in detailed:
        return None
    ...
    try:
        ...
        return None
    curr = ls.get("currentInning")
    try:
        inning = int(curr) if curr is not None else 0
    except (TypeError, ValueError):
        inning = 0
    half = str(ls.get("inningState") or "").lower()
    is_top = half.startswith("top") or bool(ls.get("isTopInning"))
    return {
        "home_runs": hr,
        "away_runs": ar,
        "inning": inning,
