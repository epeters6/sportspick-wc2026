...
_COMMAND_SYNC_MARKER = os.path.join(dp.data_dir(), "discord_command_sync.marker")
_COMMAND_SYNC_COOLDOWN_HOURS = 24.0
...
def build_mlb_signal_embed(signal: dict, status: str = "pending") -> discord.Embed:
    ...
    if signal.get("ingame_context"):
        ic = signal["ingame_context"]
        embed.title = f"⚾ MLB IN-GAME — {away} @ {home}"
        embed.add_field(
            name="Live",
            value=(
                f"Score **{ic.get('away_runs', '?')}–{ic.get('home_runs', '?')}** "
                f"· Rdiff **{ic.get('run_diff')}** · Inn **{ic.get('inning')}**"
            ),
            inline=False,
        )
    ...
    embed.add_field(
        name="Edge",
        value=f"{edge * 100:+.1f}% vs market",
        inline=True,
    )
    if signal.get("extreme_market"):
        embed.add_field(
            name="Safety",
            value="Extreme implied price - alert only; pregame autobet blocks this market.",
            inline=False,
        )
    mc = signal.get("model_confidence")
    if mc is not None:
        embed.add_field(
            name="Confidence",
            value=f"{float(mc) * 100:.0f}% (component agreement)",
            inline=True,
        )
    embed.add_field(
        name="Kelly bet",
    ...
    st = strength.upper()
    side_str = str(signal.get("recommended_side") or "yes").upper()
    fav = signal.get("favored_team") or ""
    yes_is_home = bool(signal.get("yes_is_home"))
    yes_team = home if yes_is_home else away
    no_team = away if yes_is_home else home
    bet_team = yes_team if side_str == "YES" else no_team
    side_emoji = "✅" if side_str == "YES" else "❌"
    embed.add_field(
        name="Recommendation",
        value=(
            f"**{side_emoji} BET {side_str}** on **{bet_team}** "
            f"(YES = {fav or yes_team})"
        ),
        inline=False,
    )

    footer = f"Signal: {st} ({side_str} {bet_team}) | Pavlov MLB Bot"
    if status == "placed":
...
async def send_mlb_signal(
    ...
    """Post an MLB Polymarket signal with BET IT / SKIP / BREAKDOWN (or webhook)."""
    try:
        import sys
...
def _command_sync_disabled() -> bool:
    raw = os.environ.get("DISCORD_SYNC_COMMANDS", "1").strip().lower()
    return raw in ("0", "false", "no", "off")


def _command_sync_forced() -> bool:
    raw = os.environ.get("DISCORD_FORCE_COMMAND_SYNC", "").strip().lower()
    return raw in ("1", "true", "yes", "on")


def _command_sync_recent() -> bool:
    try:
        with open(_COMMAND_SYNC_MARKER, "r", encoding="utf-8") as fh:
            ts = float(fh.read().strip())
        return (time.time() - ts) / 3600.0 < _COMMAND_SYNC_COOLDOWN_HOURS
    except (FileNotFoundError, ValueError, OSError):
        return False


def _mark_command_synced() -> None:
    os.makedirs(os.path.dirname(_COMMAND_SYNC_MARKER), exist_ok=True)
    with open(_COMMAND_SYNC_MARKER, "w", encoding="utf-8") as fh:
        fh.write(str(time.time()))


async def _sync_slash_commands_background() -> None:
    """One guild sync at most once per cooldown — avoids Discord/CF 1015 on Railway."""
    if _command_sync_disabled():
        logger.info("DiscordBot: slash sync disabled (DISCORD_SYNC_COMMANDS=0)")
        return
    if not _command_sync_forced() and _command_sync_recent():
        logger.info(
            "DiscordBot: slash sync skipped (synced within %.0fh; set DISCORD_FORCE_COMMAND_SYNC=1 to override)",
            _COMMAND_SYNC_COOLDOWN_HOURS,
        )
        return

    delay = max(5, int(os.environ.get("DISCORD_SYNC_DELAY_SECONDS", "45")))
    await asyncio.sleep(delay)

    guild_id_raw = os.environ.get("DISCORD_GUILD_ID", "").strip()
    try:
        if guild_id_raw:
            target = discord.Object(id=int(guild_id_raw))
            await tree.sync(guild=target)
            label = guild_id_raw
        elif client.guilds:
            await tree.sync(guild=client.guilds[0])
            label = str(client.guilds[0].id)
        else:
            await tree.sync()
            label = "global"
        _mark_command_synced()
        logger.info("DiscordBot: slash commands synced once (target=%s)", label)
    except discord.HTTPException as exc:
        if getattr(exc, "status", None) == 429:
            logger.error(
                "DiscordBot: Discord rate limited command sync (HTTP 429). "
                "Stop redeploying; wait before retrying. Repeated sync bursts can trigger Cloudflare 1015 on Railway egress."
            )
        else:
            logger.warning("DiscordBot: command sync failed — %s", exc)
...
async def on_ready() -> None:
    ...
    asyncio.create_task(_sync_slash_commands_background())

    logger.info(
        "DiscordBot: logged in as %s (id=%s). Channel: %s. Poly channel: %s. MLB Poly: %s. "
        "Slash sync: throttled background.",
        client.user,
    ...
    print(
        f"Discord bot ready — channel: {_channel}  |  poly: {_poly_channel}  |  "
        f"mlb_poly: {_mlb_poly_channel}  |  slash sync: throttled background"
    )
