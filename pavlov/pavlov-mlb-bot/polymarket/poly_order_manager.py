...
async def place_mlb_trade(
    signal: dict,
    auto_cap_contracts: int | None = None,
) -> dict:
    """Place a Polymarket order for an MLB moneyline signal; logs ``mlb_poly`` position.

    ``auto_cap_contracts`` — for MLB auto-bet: never exceed this many contracts; if
    Polymarket min-notional math requires more, the order is rejected (conservative).
    """
    slug = signal["ticker"]
    side = signal["recommended_side"]
    contracts = max(1, int(signal["kelly_contracts"]))
    if auto_cap_contracts is not None:
        contracts = min(contracts, int(auto_cap_contracts))
    raw_price = (
    ...
    if min_usd > 0 and contracts >= 1:
        ...
        target_usd = max(min_usd, math.ceil(raw_stake_usd - 1e-9))
        need = max(contracts, int(math.ceil(target_usd / price_prob - 1e-9)))
        if auto_cap_contracts is not None and need > int(auto_cap_contracts):
            return {
                "success": False,
                "error": (
                    "MLB auto-bet skipped: min notional would require "
                    f"{need} contracts (cap {auto_cap_contracts})."
                ),
            }
        if need > contracts:
            contracts = need
    ...
    ic = signal.get("ingame_context")
    if isinstance(ic, dict) and ic:
        entry["ingame_context"] = ic
...
def log_mlb_skip(
    signal: dict,
    *,
    learn_reason: str = "discord_mlb_skip",
    learn_source: str = "discord",
) -> bool:
    """Log a Discord skip for an MLB Polymarket signal (``venue`` = ``mlb_poly``).

    Returns True if a new learning row was appended.
    """
    extra = {
        "game_id": signal.get("game_id"),
        "home_pitcher_id": signal.get("home_pitcher_id"),
    ...
    ok = signal_learning_log.append_learning_record(
        poly_paths.SIGNALS,
        signal,
        action="skip",
    ...
    if ok:
        logger.info(
            "PolyOrderManager: MLB skip logged for %s (%s).",
            signal.get("ticker"),
            learn_reason,
        )
    return ok
