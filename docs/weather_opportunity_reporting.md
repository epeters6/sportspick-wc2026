...
## Repeated official outcome lookups

The official-label backlog now reuses a confirmed result for the same venue and exact contract within one backlog call. Only explicitly matching, resolved binary outcomes qualify. Each prediction row still receives its own metadata and database update, with the original timestamp rule. The memo is discarded when that call finishes. It does not replace the settlement process for paper positions or change any frozen strategy file.

Pending outcomes, missing results, exceptions, and mismatched market identities are not reused. They remain eligible for the existing fetch behavior, so this optimization does not postpone a newly published outcome. The backlog reports resolver calls and cache hits separately from resolved prediction rows; resolver calls do not count transport retries. This reduces duplicate lookups of already-final outcomes; it does not eliminate request-limit errors when many unresolved contracts still require separate requests. A wider request-budget or pending-result caching policy needs its own review of when outcome information becomes available to the experiment.
