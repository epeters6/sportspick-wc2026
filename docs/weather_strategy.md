...
[run_weather_cycle.py](../scripts/run_weather_cycle.py) runs through [weather_cycle.yml](../.github/workflows/weather_cycle.yml). The [Supabase dispatcher](../supabase/ops/WEATHER_DISPATCH.md) supplies the primary hourly trigger at minute 7 and a bounded minute-27 retry when the current hour has no forecast claim or completed cycle. Its separate installation starts inactive until credential setup and activation are verified. Native GitHub cron at minute 17 remains a fallback. Initial native scheduling missed eligible hours; GitHub documents that scheduled events can be delayed or dropped. [GitHub scheduling documentation](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#schedule)

The dispatcher sends an actual UTC trigger timestamp. A gate immediately before execution rejects weather requests from previous hours and requests with less than the full 25-minute job budget remaining in the hour. This applies to manual and fallback runs as well, so pre-forecast outcome processing cannot push an old hour claim into the next hour. CLV dispatches expire after five minutes. The trigger timestamp is never passed into the forecast code as its decision time. The frozen strategy, hour claims, risk limits and implementation hash are unchanged.

Local timezone checks restrict forecasting to the fixed station windows; settlement and reporting can proceed at other hours. External dispatch avoids dependence on GitHub's cron event delivery, but runner availability still matters. Database audit entries distinguish request enqueue, HTTP acceptance and errors; actual workflow completion and the durable cycle report must be checked separately.
...
The separate [GitHub CLV workflow](../.github/workflows/clv_checkpoints.yml) is the primary checkpoint observer for this forward experiment. It runs approximately every five minutes and fetches weather prices through unsigned public Kalshi and Polymarket US clients. Its timestamps retain exchange-versus-local-receipt provenance. Trading credentials are intentionally absent.

**Deployment prerequisite for a new environment:** before the first forward cycle, deploy the updated [Supabase edge observer](../supabase/functions/clv-checkpoints/index.ts) with the `metadata.experiment_id` exclusion, or disable its existing cron. The edge observer is legacy-only; an older deployed version can otherwise write incompatible checkpoint evidence for forward rows. Source edits do not update a deployed function or cron. In the existing project, version 2 was deployed and its source verified on September 6, 2026; the legacy cron was subsequently paused as part of weather-only activation.
